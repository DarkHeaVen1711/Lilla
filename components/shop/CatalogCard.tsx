...
export function CatalogCard({ product }: CatalogCardProps) {
  ...
  return (
    <article className="group rounded-[28px] border border-black/5 bg-white p-4 shadow-[0_10px_30px_rgba(0,0,0,0.06)] transition-transform duration-300 hover:-translate-y-1">
      <div className="relative aspect-[4/5] overflow-hidden rounded-[22px] bg-[#f8f7f4]">
        ...
        >
          <Heart className={`h-5 w-5 ${favorite ? "fill-[#D46B7A] text-[#D46B7A]" : ""}`} />
        </button>
        <button
          type="button"
      ...
      <div className="mt-4 flex items-center justify-between">
        <Link
          ...
          className="text-sm font-semibold text-[#D46B7A] hover:text-black"
        >
          View details
        </Link>
        {product.discount ? (
          <span className="rounded-full bg-[#FDE7EA] px-3 py-1 text-xs font-semibold text-[#D46B7A]">
            {product.discount}
          </span>
        ) : null}
      </div>
