...
export function TrustBadgesSection({ badges }: Frame525SectionProps) {
  return (
    <motion.section
      ...
      className="relative w-full bg-[#fdf6f5] font-sans py-10 md:py-[60px] transform-gpu"
    >
      <div className="max-w-[1440px] mx-auto px-5 lg:px-[60px]">
        <motion.div
          ...
          whileInView="show"
          viewport={{ once: true, margin: "-100px" }}
          className="flex flex-row justify-start sm:justify-between items-center gap-6 sm:gap-10 overflow-x-auto hide-scrollbar snap-x snap-mandatory pb-4 sm:pb-0 px-5 sm:px-0 -mx-5 sm:mx-0"
        >
          {badges.map((badge, index) => (
            <motion.div
              ...
              className="flex items-center gap-3 sm:gap-6 lg:gap-[52px] shrink-0 snap-start justify-start w-[220px] sm:w-auto transform-gpu will-change-transform"
            >
              <div className="flex-shrink-0 flex items-center justify-center text-[#D46B7A] scale-[0.8] sm:scale-100 origin-left">
                {icons[index % icons.length]}
              </div>
              <h3 className="text-black font-serif font-normal text-[18px] sm:text-2xl md:text-[28px] lg:text-[36px] leading-tight lg:leading-[44px] tracking-normal">
