...
export function DiscoverCombosSection({ products, title }: DiscoverCombosProps) {
  ...
  return (
    <motion.section
      ...
      className="relative w-full bg-[#FAFAFA] overflow-hidden font-sans py-10 md:py-[60px] transform-gpu"
    >
      <div className="max-w-[1440px] mx-auto px-5 md:px-10 lg:px-[60px] flex flex-col">
        
        {/* Title Area */}
        <div className="flex justify-center mb-8 md:mb-12">
          <h2 className="text-[#111111] text-center text-4xl md:text-[56px] font-serif font-normal leading-tight md:leading-none">
            {title}
          </h2>
        </div>
              ...
              {[...products, ...products].map((product, index) => {
                return (
                  <SwiperSlide key={`${product.id}-${index}`} className="!w-auto">
                    <div className="relative flex flex-col items-center group cursor-pointer px-4 shrink-0">
                      {/* Square Image Container */}
                      <div className="relative bg-[#F9F9F9] rounded-[24px] overflow-hidden mb-4 w-[287px] h-[287px]">
                      ...
                      {/* Title Only */}
                      <Link href={`/products/${product.slug}`} className="hover:text-[#E85A4F] transition-colors">
